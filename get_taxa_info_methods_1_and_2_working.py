...
#### stuff you need to get this to run
# in order to get taxonomy information from blast searches you need
# the taxdb.btd and taxdb.bti files from the ncbi.
# these need to be in the same location on the disk as this script otherwise it
# won't give you any taxonomy information.

# you also need the gi_taxid_nucl.dmp taxonomy/names.dmp taxonomy/nodes.dmp files,
# all from the ncbi, which can be anywhere on the disk but need to be referenced correctly
# in the arguments below.
# other than that, just the output from QIIME OTU picking remains.

# you can use this script against a custom database
# in the example below I am using the 'utax' plant database, which has been split
# into a taxonomy file and a sequence file.



# USAGE
"""
./get_taxa_info.py -i utax_taxonomy_output.tax -j ../ITS_S2SF3R/Sample_23-PW_ITS2_S2FS3R.fastq.fastaprimers_removed.fasta_otus/Sample_23.biom -k ../ITS_S2SF3R/Sample_23-PW_ITS2_S2FS3R.fastq.fastaprimers_removed.fasta_otus/new_refseqs.fna -o dummy_output.txt -r utax_sequence_file.fasta -p dummy_summary_out.txt -s 1500 -t ~/ncbi_nt/taxonomy/gi_taxid_nucl.dmp -u ~/ncbi_nt/taxonomy/names.dmp -v ~/ncbi_nt/taxonomy/nodes.dmp
"""
#FUNCTIONS
...
with open(args.output, 'w') as output_file:
    output_file.write("OTU_ID\tComment\tSequence_Counts\tRelative_Abundance(%)\tKingdom\tPhylum\tClass\tOrder\tFamily\tGenus\tSpecies\tReference_Sequence\n")
    ...
    with open("temp_blast_input.fasta", 'w') as blast_input:
        with open(args.input2, 'r') as biom_file:
            ...
            for line in biom_file:
                success = "0"
                for row in dicts:
                    # if the ID from the biom file appears in the provided taxonomy database
                    if row['ID'] == line.split("\t")[0]:
                        ...
                        output_file.write(line.split("\t")[0] + "\t" + " Taxonomy found in provided database via QIIME OTU picking" + "\t" + line.split("\t")[1].rstrip("\n") + "\t" + str(rel_abund) + "\t" + row['Taxa'].rstrip("\n").replace(";", "\t") + str(sequence) + "\n")

                        # the taxa info from the provided taxa file is then used to construct the summary stats as this is the stuff we are most confident in
                ...
                if success == "0":
                    ...
                    rel_abund = 100*(float(line.split("\t")[1])/total)
                    blast_success = "0"
                    # get the sequence associated with the new OTU from new_ref_seqs.fna
                    seq = ref_seqs_dict[line.split("\t")[0]]
                    sequence = seq.seq
                    # write the sequence into a temporary fasta file ready to be blasted against the database
                    with open("temp_seq_file.fasta", 'w') as temp_file:
                        temp_file.write(">temp_seq\n" + str(sequence))
                        temp_file.close
                    # blast the sequence file against the sequences provided by the utax database
                    ...
                    if os.path.isfile("temp_blast_out.xml"):
                        if(int(file_len("temp_blast_out.xml", -1)) > 5):
                            ...
                            with open("temp_blast_out.xml", 'r') as blast_output:
                                ...
                                # if it has found a sufficiently similar result in the blast search
                                # then it greps the ID of the sequence against the taxonomy file
                                # and retrieves the taxonomy info
                    ...
                    if blast_OTU_success == "1":
                        output_file.write(output_string.split("\t")[0] + "\t" + " Taxonomy assigned by blast against provided database" + \
                                                    "\t" + line.split("\t")[1].rstrip("\n") + \
                                                    "\t" + str(rel_abund) + \
                                                    "\t" + output_string.split("\t")[1].rstrip("\t").replace(";", "\t") + \
                                                    str(sequence).lstrip("\t") + \
...
# the total number of lines in the biom file is the same as the total of the:
# a) OTU assigned taxonomy by QIIME
# b) assigned taxonomy by blast to database
# c) written out to temp_blast_input because does not yet have a taxonomy
...
if os.path.isfile("temp_blast_out2.xml"):
    ...
    #                e_val = line.split("\t")[2]
    #                Bit = line.split("\t")[3]
    #                perc_id = line.split("\t")[4]
    #                perc_cov = line.split("\t")[5]
    #                spec_name = line.split("\t")[6]
    #                taxa_id = line.split("\t")[7]
...
print(len(list_of_data_to_blast, blast_results))
#os.remove("temp_blast_out2.xml")
#blast_results looks like:
# Avicennia officinalis 948275600 0	722	95.56	100 New.ReferenceOTU0
data_present = []
empty_data = []
get_taxonomy = []
...
if os.path.isfile("taxonomy_output.txt"):
    ...
    for item, row in zip(data_present, taxonomy_info):
        # attempt to handle some of the inconsistencies in the taxonomy data_present
        # empty fields etc.
        ...
        if row.split("\t")[3] == "No taxonomy information found" or len(row.split("\t")) < 16:
            ...
            output.append('\t'.join([item.split("\t")[0],
                                    item.split("\t")[1],
                                    item.split("\t")[2],
                                    "k__" + row.split("\t")[5] + "; p__" + row.split("\t")[7] + "; c__" + row.split("\t")[9] + "; o__" + row.split("\t")[11] + "; f__" + row.split("\t")[13] + "; g__" + row.split("\t")[15] + "; s__" + row.split("\t")[15].lstrip("n"),
                                    "Percent_similarity",
                                    item.split("\t")[7],
                                    "Percent_alignment_coverage",
                                    item.split("\t")[8].rstrip("\n"),
...
##### second round of collecting taxa counts
# this time using the info from all 3 processes above
...
# get length of file
file_length =  file_len(args.output, -1)
...
with open(args.output, 'r') as input_file:
    ...
    for line in input_file:
        if int(count) < int(file_length - 1):
            ...
            #if taxa == "No BLAST result found." or taxa == "No taxonomy information found in Blast Database.":
                continue
            else:
